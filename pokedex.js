...
// function loadMoves(pokemonMoves,fetchingofmoves) {
        
//         let innerElement = "";
//         pokemonMoves.forEach((item) => {
//             innerElement += `
//             <li>${item}</li>`;
//         });
//     fetchingofmoves.innerHTML = innerElement;
// }
...
async function Pokecheck(apiurl1) {
    ...
    if (pokeImage.src === "https://radha231.github.io/PokeDex/null") {
    ...
    // const pokemoves = data.moves.map(pokemon => pokemon.move.name);
    // const pokemoveurl = data.moves.map(pokemon => pokemon.move.url);
    // const result = await fetch(pokemoveurl);
    // var pokemovedescription = await result.json();
    // var pokemovesdef = pokemovedescription.flavor_text_entries.find(moveInfo => moveInfo.language.name === "en").flavour_text;
    // let fetchingdesc = document.querySelector(".movedescription");

    // let fetchingMoves=document.querySelector(".movename")
    // loadMoves(pokemovesdef, fetchingdesc);
